...
public class ModifiableCrossbowItem extends ModifiableLauncherItem {
  ...
  public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
    ...
    if (heldAmmo.isEmpty()) {
      ...
      if (!ammo.isEmpty() || tool.getModifiers().has(TinkerTags.Modifiers.CHARGE_EMPTY_BOW_WITH_DRAWTIME)) {
        ...
        if (!ammo.isEmpty()) {
          if (storeDrawingItem) {
            persistentData.put(KEY_DRAWBACK_AMMO, ammo.save(player.level().registryAccess(), new CompoundTag()));
          } else {
  ...
  public List<Component> getStatInformation(IToolStackView tool, @Nullable Player player, List<Component> tooltips, TooltipKey key, TooltipFlag tooltipFlag) {
    ...
    // 1.21: parsing a stack from NBT needs registry access; only available here via the (client) player
