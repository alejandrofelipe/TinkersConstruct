...
public class SmelteryRecipeBuilder {
  ...
  private void oreMelting(float scale, String tagPrefix, @Nullable TagKey<Item> size, float factor, String output, boolean forceOptional) {
    ...
    if (oreByproducts.length == 0) {
      ...
      // PORT M3: NeoForge 1.21 removed ConditionalRecipe (multiple alternatives under one id).
      // Emit each byproduct variant as its own recipe id, guarded by its condition via ConsumerWrapperBuilder.
      ...
      for (IByproduct byproduct : oreByproducts) {
        ...
        ICondition condition = alwaysPresent ? TrueCondition.INSTANCE : tagCondition("ingots/" + byproduct.getName());
        RecipeOutput variantOutput = ConsumerWrapperBuilder.wrap().addCondition(condition).build(wrapped);
        supplier.get()
                .addByproduct(byproduct.getFluid(scale))
                .setOre(oreRate, byproduct.getOreRate())
                .save(variantOutput, location.withSuffix("_" + index));
        index++;
        if (alwaysPresent) {
          break;
        }
      ...
      if (!alwaysPresent) {
        RecipeOutput variantOutput = ConsumerWrapperBuilder.wrap().addCondition(TrueCondition.INSTANCE).build(wrapped);
        supplier.get().save(variantOutput, location.withSuffix("_" + index));
      }
    }
  }
