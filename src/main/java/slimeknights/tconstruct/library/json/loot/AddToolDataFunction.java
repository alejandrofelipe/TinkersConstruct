...
public class AddToolDataFunction extends LootItemConditionalFunction {
  ...
  /**
   * Codec for the function, serializing the optional damage percent and material list alongside the standard
   * conditional-function fields.
   */
  public static final MapCodec<AddToolDataFunction> CODEC = RecordCodecBuilder.mapCodec(inst ->
    commonFields(inst).and(inst.group(
      RandomMaterial.LOADER.list(0).codec().optionalFieldOf("materials", List.of()).forGetter(fn -> fn.materials),
      Codec.floatRange(0, 1).optionalFieldOf("damage_percent", 0f).forGetter(fn -> fn.damage)
    )).apply(inst, (conditions, materials, damage) -> new AddToolDataFunction(conditions, damage, materials)));
