...
public interface EnchantmentModule extends ModifierModule, LevelingIntModule, ConditionalModule<IToolStackView> {
  /* Common fields */
  LoadableField<ResourceKey<Enchantment>,EnchantmentModule> ENCHANTMENT = Loadables.ENCHANTMENT.requiredField("name", EnchantmentModule::enchantment);
  ...
  class Constant implements EnchantmentModule, EnchantmentModifierHook {
    ...
    @Deprecated(forRemoval = true)
    public Constant(ResourceKey<Enchantment> enchantment, int level, ModifierCondition<IToolStackView> condition) {
      this(enchantment, LevelingInt.eachLevel(level), condition);
    }

    /** @deprecated use {@link Builder#constant()} */
    @Deprecated(forRemoval = true)
    public Constant(ResourceKey<Enchantment> enchantment, int level) {
  ...
  class Protection extends Constant implements ProtectionModifierHook {
    ...
    protected Protection(ResourceKey<Enchantment> enchantment, LevelingInt level, ModifierCondition<IToolStackView> condition) {
      super(enchantment, level, condition);
    }

    /** @deprecated use {@link Builder#protection()} */
    @Deprecated(forRemoval = true)
    public Protection(ResourceKey<Enchantment> enchantment, int level, ModifierCondition<IToolStackView> condition) {
      super(enchantment, level, condition);
    }
    ...
    public float getProtectionModifier(IToolStackView tool, ModifierEntry modifier, EquipmentContext context, EquipmentSlot slotType, DamageSource source, float modifierValue) {
      if (condition().matches(tool, modifier)) {
        ...
        // PORT M3: Enchantment.slots and Enchantment#getDamageProtection were removed in 1.21.1 (protection is now a data-driven
        // EnchantmentValueEffect). Recomputing the subtracted protection requires a Holder<Enchantment> and RegistryAccess that are
        // not available here. Protection cancellation is temporarily disabled.
        if (subtractLevel > 0 && !source.is(DamageTypeTags.BYPASSES_ENCHANTMENTS)) {
          // modifierValue -= <enchantment damage protection>;
        }
      }
   ...
   */
  record MainHandHarvest(ResourceKey<Enchantment> enchantment, LevelingInt level, ModifierCondition<IToolStackView> condition, ResourceLocation conditionFlag, IJsonPredicate<BlockState> block, IJsonPredicate<LivingEntity> holder) implements EnchantmentModule, EnchantmentModifierHook, BlockHarvestModifierHook {
    ...
    @Deprecated(forRemoval = true)
    public MainHandHarvest(ResourceKey<Enchantment> enchantment, int level, ModifierCondition<IToolStackView> condition, ResourceLocation conditionFlag, IJsonPredicate<BlockState> block, IJsonPredicate<LivingEntity> holder) {
   ...
   */
  record ArmorHarvest(ResourceKey<Enchantment> enchantment, LevelingInt level, ModifierCondition<IToolStackView> condition, Set<EquipmentSlot> slots, IJsonPredicate<BlockState> block, IJsonPredicate<LivingEntity> holder) implements EnchantmentModule, HarvestEnchantmentsModifierHook {
    ...
    @Deprecated(forRemoval = true)
    public ArmorHarvest(ResourceKey<Enchantment> enchantment, int level, ModifierCondition<IToolStackView> condition, Set<EquipmentSlot> slots, IJsonPredicate<BlockState> block, IJsonPredicate<LivingEntity> holder) {
