...
public class CreativeSlotItem extends Item {
  ...
  public static SlotType getSlot(ItemStack stack) {
    ...
    if (data != null && data.getUnsafe().contains(NBT_KEY, Tag.TAG_STRING)) {
      return SlotType.getIfPresent(data.getUnsafe().getString(NBT_KEY));
    }
    return null;
  }
  ...
  public static ItemStack withSlot(ItemStack stack, SlotType type) {
    CustomData.update(DataComponents.CUSTOM_DATA, stack, tag -> tag.putString(NBT_KEY, type.getName()));
    return stack;
  }
